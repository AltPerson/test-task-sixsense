...
import { isSearchJob, type SearchJob } from "@/features/search/model/search-job";
import {
  isSearchResultsPage,
...
async function fetchJson<T>(
  ...
  const response = await requestWithTimeout(url, { method: "GET" }, options);
...
export function fetchSearchMetadata(
  options: SearchRequestOptions = {},
): Promise<SearchMetadata> {
  return fetchJson("/api/search/metadata", isSearchMetadata, options);
}

export async function fetchEnumCatalog(
  name: string,
  options: SearchRequestOptions = {},
): Promise<EnumCatalog> {
  ...
  try {
    return await fetchJson(url, isEnumCatalog, options);
  } catch (error) {
    if (
      !(error instanceof SearchApiError) ||
      error.status !== 503 ||
    ...
    return fetchJson(url, isEnumCatalog, options);
  }
}
