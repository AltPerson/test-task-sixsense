...
import { fetchEnumCatalog } from "@/features/search/api/search-api";
import type {
  EnumOption,
  FieldMetadata,
  SensorMetadata,
} from "@/features/search/model/metadata";
import type { SearchConditionDefinition } from "@/features/search/model/search-definition";
...
export function ConditionEditor({
  ...
  const enumQuery = useQuery({
    ...
    queryFn: ({ signal }) =>
      fetchEnumCatalog(field?.enumName ?? "", { signal }),
  ...
  return (
    <fieldset className="rounded-xl border border-slate-200 bg-slate-50 p-4">
      <legend className="sr-only">Search condition for {field.label}</legend>
