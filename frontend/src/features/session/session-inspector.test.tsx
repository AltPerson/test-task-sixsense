...
import { encodeSearchDefinition } from "@/features/search/search-definition";
import { SessionApiError } from "@/features/session/session-api";
import { SessionInspector } from "@/features/session/session-inspector";
import type { ProtocolSchema, SessionDetail } from "@/features/session/session-detail";
...
vi.mock("@/features/session/session-api", async (importOriginal) => {
  const actual = await importOriginal<typeof import("@/features/session/session-api")>();
...
vi.mock("@/features/search/search-api", () => ({
  fetchSearchMetadata: api.fetchSearchMetadata,
}));
...
describe("SessionInspector", () => {
  ...
  it("shows a recoverable protocol-schema error", async () => {
    api.fetchProtocolSchema.mockRejectedValue(new SessionApiError(503, "unavailable", "Unavailable"));
    renderInspector();
